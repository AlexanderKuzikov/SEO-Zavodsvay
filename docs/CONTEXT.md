...
| Семантическое ядро | расширено | `data/core/core.csv` — 161 запрос (было 54): 104 коммерческих, 50 инфо, 7 локальных; частота у 118. Билдер: `scripts/build_core.py` (ядро + projects.json + курируемые добавления) |
...
| Связь с SerpWatcher | merged | 62 → 161 запрос в `data/projects.json` (все daily, region 50, freq из Wordstat) |
...
| 2026-08-05 | Ядро расширено до 161 запроса: `build_core.py` (мерж ядра + 62 из projects.json + 99 курируемых из merged, частоты у 118). `gen_serpwatcher.py --merge`: projects.json 62 → 161 запрос, все daily, targetUrl у всех |
