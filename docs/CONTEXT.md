...
| Вебмастер-монитор | создан | `scripts/webmaster_monitor.py` (174 запроса × регион 11108/все × desktop/mobile, `data/webmaster/data.json`, окно 14д, срез-замена) + UI `ui/` (vanilla + uPlot, светлая тема, дизайн-токены) + `serve_ui.py` (порт 8794). Статус: `no-data` — ждём первый обход сайта (~1–2 дня) |
...
| 6 | med | Вебмастер-монитор: после первого обхода запустить `webmaster_monitor.py` повторно — ждём реальные позиции; настроить Task Scheduler (ежедневно 07:00) |
